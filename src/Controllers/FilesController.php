...
class FilesController extends BaseController
{
    ...
    {
        try {
            $imageProcessor = app(\Glueful\Services\ImageProcessorInterface::class);

            $width = isset($params['w']) ? (int)$params['w'] : null;
            $height = isset($params['h']) ? (int)$params['h'] : null;
            $quality = (int)($params['q'] ?? 85);
            ...
            if ($zoom && $zoom !== 100) {
                ...
                if ($processor->width() && $processor->height()) {
                    $processor->resize(
                        (int)($processor->width() * $scaleRatio),
                        (int)($processor->height() * $scaleRatio)
                    );
                }
            }

            $imageData = $processor->cached()->getImageData();
            $mimeType = $processor->getMimeType();
