...
final class ConsoleProvider extends BaseServiceProvider
{
    ...
    private function getCommands(): array
    {
        ...
        // The manifest is also stamped with the framework version that wrote it: every entry of an
        // older manifest can still exist while a command the upgrade added is missing from it.
        if ($isProduction && file_exists($cacheFile)) {
            $cached = require $cacheFile;
            if (
                is_array($cached)
                && ($cached['framework'] ?? null) === Version::getVersion()
                && is_array($cached['commands'] ?? null)
            ) {
                $listed = $cached['commands'];
                $valid = array_values(array_filter($listed, static fn ($c): bool => is_string($c) && class_exists($c)));
                if ($valid !== [] && count($valid) === count($listed)) {
                    return $valid;
                }
            }
        }
    ...
    private function writeCache(string $cacheFile, array $commands): void
    {
        $content = "<?php\n\n// Auto-generated by ConsoleProvider - do not edit\n// Generated: "
            . date('Y-m-d H:i:s') . "\n\nreturn "
            . var_export(['framework' => Version::getVersion(), 'commands' => $commands], true) . ";\n";
