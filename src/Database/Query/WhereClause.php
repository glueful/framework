    ...
    public function getConditionsArray(): array
    {
        ...
        foreach ($this->conditions as $condition) {
            ...
            if ($condition['type'] === 'basic') {
                ...
                $sql = $condition['sql'] ?? '';
                if (preg_match('/^(.+?)\s+([=!<>]+|LIKE|NOT LIKE|IN|NOT IN|IS|IS NOT)\s+\?$/i', $sql, $matches) === 1) {
                ...
                $sql = $condition['sql'] ?? '';
                if (preg_match('/^(.+?)\s+IS\s+(NOT\s+)?NULL$/i', $sql, $matches) === 1) {
                    $column = trim($matches[1], '`"[]');
                    if (strpos($column, '.') !== false) {
                        $parts = explode('.', $column);
                        // Re-strip wrap chars: trimming the full qualified identifier
                ...
                $sql = trim((string) ($condition['sql'] ?? ''));

                // Handle raw null checks generated by whereNull()/whereNotNull()
                if (preg_match('/^(.+?)\s+IS\s+(NOT\s+)?NULL$/i', $sql, $matches) === 1) {
                    $column = trim($matches[1], '`"[]');
                    if (strpos($column, '.') !== false) {
                        $parts = explode('.', $column);
                ...
                if (preg_match('/^(.+?)\s+(NOT\s+)?IN\s*\(\s*\?(?:\s*,\s*\?)*\s*\)$/i', $sql, $matches) === 1) {
                    $column = trim($matches[1], '`"[]');
                    if (strpos($column, '.') !== false) {
                        $parts = explode('.', $column);
        ...
        if (count($complexConditions) > 0) {
            $preview = json_encode(array_slice($complexConditions, 0, 1), JSON_UNESCAPED_SLASHES);
            $msg = 'Complex WHERE conditions (OR, NOT, nested/raw unsupported) '
                . 'are not yet supported for UPDATE/DELETE operations';
            throw new \RuntimeException(
                $msg . ($preview !== false ? " | first_condition={$preview}" : '')
            );
        }
    ...
    protected function addCondition(
        ...
        if (is_array($column)) {
            foreach ($column as $col => $val) {
                ...
                if ($val === null) {
                    $this->addNullCondition((string) $col, $boolean);
                } else {
                    $this->addBasicCondition((string) $col, '=', $val, $boolean);
                }
            }
            return;
        }
        ...
        if ($operator !== null && preg_match('/^[<>=!]+$|^\w+\s+\w+$/i', $operator) === 1) {
