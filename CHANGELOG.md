...
### Removed
- **OpenAPI: the legacy `comments` (docblock-parsing) generator has been removed; `reflect` is now the only generator.** The `CommentsDocGenerator` (PHPDoc/`@route`/`@response` docblock and JSON-fragment route parsing) is deleted, along with the `documentation.generator` config switch and the `API_DOCS_GENERATOR` env var — the code-first `reflect` generator (default since the previous release) is now unconditional. `OpenApiGenerator` derives the spec's `paths` solely from the live route table; its constructor no longer accepts a `CommentsDocGenerator` argument (signature is now `(ApplicationContext $context, ?DocGenerator $docGenerator = null, ?FileFinder $fileFinder = null, bool $runFromConsole = false, ?ResourceRouteExpander $resourceExpander = null)`), and the `getCommentsGenerator()` accessor and `processExtensionAndRouteDocs()` orchestration are gone. **Preserved:** resource-route expansion (`include_resource_routes`) and merging of hand-authored custom OpenAPI JSON fragments under `docs/json-definitions/` (`DocGenerator::generateFromDocJson`/`generateFromExtensionFiles`/`generateFromRouteFiles`) — these are independent of docblock parsing and still run. The `@*` route docblocks in `routes/*.php` are now inert (no longer parsed) but harmless. Operators who set `API_DOCS_GENERATOR` should remove it; the value is ignored.

## [1.56.0] - 2026-06-13 — Rastaban

> **Theme: Security hardening pass (continued).** The second wave of the June 2026 security review — queue/scheduler payload signing, SSRF-safe HTTP with validated-DNS pinning, unified sensitive-parameter redaction, fail-closed CORS/image defaults, and JWT temporal-claim enforcement. Almost entirely fixes, but several change defaults and add config/env vars — see **Upgrade Notes** — so this ships as a minor. Staying in 1.x per the pre-public policy.
...
### Upgrade Notes
- **CORS now fails closed.** The standalone `Glueful\Http\Cors` handler no longer defaults to allowing all origins, and `CORS_SUPPORTS_CREDENTIALS` now defaults to `false` (previously effectively `true`). Apps relying on the old open/credentialed defaults must set `CORS_ALLOWED_ORIGINS` and, if required, `CORS_SUPPORTS_CREDENTIALS=true`. The wildcard-origin + credentials combination is now refused at emit time regardless of config.
- **Remote image fetching is opt-in.** With no `image.security` config present, `ImageSecurityValidator` now defaults to an empty allow-list with external URLs disabled. Configure `image.security.allowed_domains` (or install/configure `glueful/media`) to fetch remote images.
- **Queue & scheduler payloads are HMAC-signed by default.** New keys `QUEUE_PAYLOAD_SIGNING` (default `true`) and `QUEUE_REQUIRE_SIGNED_PAYLOADS` (default `true`) gate signing/verification; signing is inert without an `APP_KEY`. Deployments draining legacy unsigned rows can temporarily set `QUEUE_REQUIRE_SIGNED_PAYLOADS=false`. Stored queue/scheduler handler classes must now implement the queue contract (`JobInterface`) to be instantiated.
- **Memcached cache format changed.** Memcached now stores `SecureSerializer`-encoded strings; raw legacy *string* values that aren't valid serialized data now throw on read — flush the cache when upgrading a Memcached-backed deployment.
- **JWT temporal claims are required.** Tokens without `exp` (or with expired/non-numeric `exp`, future `nbf`, or future/non-numeric `iat`) are now rejected. A token minted without an expiry no longer validates.
- **Trusted proxies are honored.** Set `TRUSTED_PROXIES` (comma-separated) so `Request::getClientIp()` resolves the real client behind load balancers; otherwise client IPs collapse to the proxy address. New optional `http.safe_fetch.max_redirects` (default `3`) bounds SSRF-safe fetch redirects. No migrations this release.
