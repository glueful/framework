...
**Status:** COMPLETE — `reflect` is now the canonical and ONLY OpenAPI generator. Phase 0 (`6822eb3`, `8b7f97b`), Phase 1 (`8e846ff`), Phase 2 schema inference (`23ed4d3`, `f0b83be`), and **Phase 3 (make `reflect` canonical, remove `comments`)** are all shipped. Phase 3 — see `docs/implementation-plans/openapi-reflect-canonical-phase-3.md` — added a minimal typed override surface (`#[ApiOperation]`, `#[QueryParam]`, `#[ApiRequestBody]` for multipart + JSON-DTO-class doc-only bodies, `#[ApiResponse]` `body:` for non-JSON), migrated all 5 route-wired controllers off `@*` docblocks onto types+attributes (each with a per-route characterization test), proved `reflect ⊇ comment` over the live route table (0 shortfalls, 32 operations), then deleted `CommentsDocGenerator` + the `documentation.generator` switch + the route docblocks. **The comment-path items #5/#6 (`@example` brace fix, docblock grammar) and #8 (parser consolidation) were SUPERSEDED — rather than polish/refactor the regex parser, the comment generator was removed entirely** (consolidation-by-deletion). The reflect-only model: types are the source of truth; attributes document only what types can't express (prose/tags, multipart, binary/non-JSON responses, doc-only JSON shapes for manual handlers).
