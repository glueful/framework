...
1. **`#[ApiRequestBody]` is multipart / non-JSON ONLY — never for JSON.** A JSON request body, even a polymorphic or free-form one, uses a typed DTO (e.g. `ResourceCreateData`/`ResourceUpdateData`/`BulkDeleteData`/`BulkUpdateData`, each with `array<string,mixed>` fields where the shape is dynamic). The inline-`schema` array on `#[ApiRequestBody]` exists solely because a multipart/binary body cannot be a JSON DTO.
