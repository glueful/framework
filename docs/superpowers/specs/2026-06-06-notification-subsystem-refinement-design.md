...
7. **Registry naming:** keep current methods now; longer-term add clearer `getRegisteredChannelNames()` / `getActiveChannelNames()` with the old methods kept as aliases. (`getAvailableChannels()` = registered names, `:117`; `getActiveChannels()` = `isAvailable()`-filtered, `:127`.)
...
- Everything is additive/internal — **no breaking public-contract changes this pass.** `send(): bool`, the queue fallback, and the `true` capability default all stay.
