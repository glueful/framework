...
> 3. **Priority — not the filename — carries cross-source order (confirmed in Phase 4).** `FileFinder::findMigrations()` only accepts basenames matching `^\d{3}_` (exactly three digits). So a package cannot "win" ordering by using a longer numeric prefix (e.g. `0001_`); every source uses `001_`-style names and the **priority** is the sole cross-source ordering lever. `glueful/users` ships `001_…005_` at `IDENTITY` priority and still sorts ahead of the skeleton's `001_` at `DEFAULT`; the `source` column keeps the same-basename rows distinct. (Any doc examples here showing `0001_` are illustrative only — real files are 3-digit.)
