...
- 🟡 **Task 4 — TwoFactorService** (`src/Auth/TwoFactor/TwoFactorService.php`). Production code complete, lint + PHPStan clean. Verified `OTP::generateNumeric/hashOTP/verifyHashedOTP`, `TokenManager::createUserSession(array,?string):array`, `NotificationService::send(...)`, `config()` signatures all match the plan. **Remaining: Task 4 Step 4 integration test not yet written.**

**Remaining:** Task 4 Step 4 (integration test), Tasks 5–11 (controller, routes, container+config, CLI, email template, the core-auth refactor in Task 10, and final verification).

**Test-harness notes for the remainder:** builder-level tests use `new Connection(['engine'=>'sqlite','sqlite'=>['primary'=>$tmpFile],'pooling'=>['enabled'=>false]])`; `JWTService` key is set via reflection on the private static `$key` (`$algorithm` defaults to HS256); `ApplicationContext` is path-constructible without a full boot; `TokenManager` and `NotificationService` are non-final (mockable via `createMock`).
